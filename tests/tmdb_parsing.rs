...
fn test_parse_tv_search_results() {
    let data: serde_json::Value =
        serde_json::from_str(&fs::read_to_string("tests/fixtures/tmdb/search_tv.json").unwrap()).unwrap();
...
fn test_parse_movie_search_results() {
    let data: serde_json::Value =
        serde_json::from_str(&fs::read_to_string("tests/fixtures/tmdb/search_movie.json").unwrap()).unwrap();
    let results = data["results"].as_array().unwrap();
    assert_eq!(results.len(), 2);
...
fn test_parse_season_detail() {
    let data: serde_json::Value =
        serde_json::from_str(&fs::read_to_string("tests/fixtures/tmdb/season_detail.json").unwrap()).unwrap();
